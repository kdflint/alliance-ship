...
In your development IDE (or just a plain text editor), open the project source code file `<project-root>/alliance-community/alliance/config/local_settings_template.py`

Make a copy of this file in the same directory, naming it `local_settings.py`. Edit the values for `SOCIAL_AUTH_GITHUB_KEY` and `SOCIAL_AUTH_GITHUB_SECRET` so that these values match the values that were generated by GitHub after you executed Step 3a.
...
Navigate to your GitHub profile. Use this vagrant share designator to add a fully qualified URL in the Authorization callback field of your OAuth application settings (the field we left empty in step 3a).
